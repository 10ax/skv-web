...
const Contact = () => {
  ...
  const validatePhone = (phone: string): boolean => {
    ...
    const phoneRegex = /^(\+39\s?)?((3[0-9]{2}\s?\d{3}\s?\d{4})|(0[0-9]{1,3}\s?\d{3,4}\s?\d{3,4}))$/;
  ...
  const validateVATNumber = (vat: string): boolean => {
    ...
    const digits = vat.split('').map(d => parseInt(d));
    let sum = 0;
    for (let i = 0; i < 10; i++) {
  ...
  const validateForm = (formData: FormData): ValidationErrors => {
    ...
    if (activeTab === 'business') {
      ...
      if (vatNumber && !validateVATNumber(vatNumber)) {
        newErrors.vat_number = 'Inserisci una Partita IVA italiana valida (11 cifre)';
      }
    }
  ...
  const renderPrivateForm = () => (
    <>
      <div className="text-left">
        <label htmlFor="name" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Nome:</label>
        <input
            type="text"
            id="name"
            name="name"
      ...
      <div className="text-left">
        <label htmlFor="surname" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Cognome:</label>
        <input
            type="text"
            id="surname"
            name="surname"
      ...
      <div className="text-left">
        ...
        <input
            ...
            className={`w-full py-3 px-4 bg-gray-50 dark:bg-gray-700 border ${errors.email ? 'border-red-500' : 'border-border dark:border-gray-600'} rounded-lg text-base text-gray-900 dark:text-gray-50 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary`}
        />
        {errors.email && <p className="mt-1 text-sm text-red-500">{errors.email}</p>}
      </div>

      <div className="text-left">
        <label htmlFor="phone" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Telefono:</label>
        <input
            type="tel"
            id="phone"
            name="phone"
      ...
      <div className="text-left">
        <label htmlFor="message" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Messaggio:</label>
        <textarea
            id="message"
            name="message"
            required
  ...
  const renderBusinessForm = () => (
    <>
      <div className="text-left">
        <label htmlFor="company_name" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Ragione Sociale:</label>
        <input
            type="text"
            id="company_name"
            name="company_name"
      ...
      <div className="text-left">
        <label htmlFor="vat_number" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Partita IVA:</label>
        <input
            type="text"
            id="vat_number"
            name="vat_number"
        ...
        />
        {errors.vat_number && <p className="mt-1 text-sm text-red-500">{errors.vat_number}</p>}
      </div>

      <div className="text-left">
        <label htmlFor="contact_person" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Persona di Contatto:</label>
        <input
            type="text"
            id="contact_person"
            name="contact_person"
      ...
      <div className="text-left">
        <label htmlFor="address" className="block mb-2 font-semibold text-gray-700 dark:text-gray-300">Indirizzo:</label>
        <input
            type="text"
            id="address"
            name="address"
    ...
    return  (<div className="flex justify-center items-center p-8 bg-gray-100 dark:bg-gray-900">
      <div className="bg-background dark:bg-gray-800 p-10 rounded-xl shadow-lg max-w-2xl w-full text-center">
        <h2 className="text-4xl font-bold mb-4 text-gray-800 dark:text-white">Contattaci</h2>
        <p className="mb-8 text-tertiary dark:text-gray-400">Scegli il tipo di cliente e compila il modulo per essere contattato!</p>
